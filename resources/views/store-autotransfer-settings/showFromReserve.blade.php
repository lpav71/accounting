...
@section('content')
    ...
    <div class="row">
        ...
        <input type="hidden" name="store_id_from" value="{{$reserveStore->id}}">
        <input type="hidden" name="store_id_to" value="{{$mainStore->id}}">
        <table class="col-xs-12 col-sm-12 table">
            <thead>
            <tr>
                <th scope="col" colspan="9"><strong>{{ __('Amount in transfer reserve') }}</strong>
                    {{ $amountBack }}</th>
            </tr>
            <tr>
            ...
            <tbody>
            @foreach($transferBackProducts as $key => $transferProduct)
                <tr>
                    <th scope="col">{{$key}}</th>
