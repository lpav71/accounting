...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>{{ __('Edit store auto transfer setting') }}</h2>
            </div>
    ...
    @endif
    {!! Form::open( ['method' => 'PATCH','route' => ['store-autotransfer-settings.update', $setting->id]]) !!}
    <div class="row">
        <div class="col-12 col-sm-6">
            <div class="form-group">
                {!! Form::label('name', __('Name:')) !!}
                {!! Form::text('name', $setting->name, ['placeholder' => __('Name'),'class' => 'form-control']) !!}
            </div>
        </div>
        <div class="col-12 col-sm-6">
            <div class="form-group">
                {!! Form::label('max_amount', __('Max amount')) !!}
                {!! Form::text('max_amount', $setting->max_amount, ['placeholder' => __('Max amount'),'class' => 'form-control']) !!}
            </div>
        </div>
        <!---->
        <div class="col-12 col-sm-4">
            <div class="form-group">
                ...
                {!! Form::text('max_day', $setting->max_day, ['placeholder' => __('Max day'),'class' => 'form-control']) !!}
            </div>
        </div>
        <div class="col-12 col-sm-4">
            <div class="form-group">
                {!! Form::label('min_day', __('Min day')) !!}
                {!! Form::text('min_day', $setting->min_day, ['placeholder' => __('Min day'),'class' => 'form-control']) !!}
            </div>
        </div>
        <div class="col-12 col-sm-4">
            <div class="form-group">
                {!! Form::label('latest_sales_days', __('The number of days for which sales are counted')) !!}
                {!! Form::text('latest_sales_days', $setting->latest_sales_days, ['placeholder' => __('The number of days for which sales are counted'),'class' => 'form-control']) !!}
            </div>
        </div>
        <!---->
        <div class="col-xs-12 col-sm-6">
            <div class="form-group">
                ...
                {!! Form::select('main_store_id', $stores, $setting->main_store_id, ['class' => 'form-control selectpicker']) !!}
            </div>
        </div>
        <div class="col-xs-12 col-sm-6">
            <div class="form-group">
                {!! Form::label('reserve_store_id', __('Reserve store')) !!}
                {!! Form::select('reserve_store_id', $stores, $setting->reserve_store_id, ['class' => 'form-control selectpicker']) !!}
            </div>
        </div>
        <div class="col-12">
