...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
            @if(preg_match('/^hidden-orders\./', Route::currentRouteName()))
            <h4 class="d-inline">{{ __('Orders Management - Hidden') }}</h4>
            @else
            <h4 class="d-inline">{{ __('Orders Management')}}</h4> 
            <table class="d-inline ml-3">
                <tr>
                    <td class="pr-1 align-baseline"><a href="{{ route('orders.index', ['redClients'=>1]) }}"><span class="mr-1">{{__('Unconfirmed clients:')}}</span></a><span class="mr-2">{{$redClients}}</span></td>
                    <td class="pr-1 align-baseline"><span class="mr-1">{{__('Expired tasks:')}}</span><span class="mr-2">{{$expiredTasks}}</span></td>
                    <td class="pr-1 align-baseline"><span class="mr-1">{{__('Actual tasks:')}}</span><span class="mr-2">{{$tasksActual}}</span></td>
                    <td class="pr-1 align-baseline"><span class="mr-1">{{__('Orders to pack:')}}</span><span class="mr-2">{{$confirmedOrders}}</span></td>
                    <td class="pr-1 align-baseline"><span class="mr-1">{{__('Self shipping')}}:</span><span class="mr-2">{{$selfShipping}}</span></td>
                </tr>
            </table>
               @endif
            </div>
            <div class="pull-right">
                @can('order-create')
                    <a class="btn btn-sm btn-success" href="{{ route('orders.create') }}"> {{ __('Create New Order') }}</a>
                @endcan
            </div>
        </div>
    </div>
    ...
    <table class="table table-light table--responsive table-sm table-bordered table-striped small-1 order-table">
        ...
        <tr id="searchOrders">
            ...
            <td class="p-0" area-label="{{ __('Comment') }}">{{ Form::text('comment', Request::input('comment'), ['class' => 'form-control form-control-sm search-input']) }}</td>
            ...
            <td class="text-center p-1">
                <div>
                    {!! Form::hidden('prohibition', '100', ['placeholder' => __('Name'),'class' => 'form-control']) !!}
                </div>
        ...
        @foreach ($orders as $key => $order)
            <tr style="background-color: {{ !empty($order->currentState())?$order->currentState()['color']: '#FFFFFF' }};">
                ...
                <td area-label="{{ __('Comment') }}">{!! preg_replace('|http[s]{0,1}://([a-zA-Z0-9-./_\?\+=&%]+)|', '<a href="http://$1" target="_blank"><i class="fa fa-link text-dark"></i></a>', $order->comment ) !!}
