...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>{{ __('Edit Route List State') }}</h2>
            </div>
    ...
    @endif
    {!! Form::model($routeListState, ['route' => ['route-list-states.update', $routeListState->id],'method'=>'PATCH']) !!}
    <div class="row">
        <div class="col-xs-12 col-sm-6">
            ...
            <div class="form-group form-check">
                ...
                {!! Form::label('is_successful', __('Is successful')) !!}
            </div>
            <div class="form-group form-check">
...
{{--                {!! Form::select('previous_states_id[]', $routeListStates, $routeListState->previousStates->pluck('id'), ['multiple' => true, 'class' => 'form-control selectpicker']) !!}--}}
{{--            </div>--}}
{{--        </div>--}}
        <div class="col-xs-12 col-sm-6">
            <div class="form-group">
                {!! Form::label('need_order_detail_state_id[]', __('Need Order Detail states:')) !!}
                {!! Form::select('need_order_detail_state_id[]', $orderDetailStates, $routeListState->needOrderDetailStates->pluck('id'), ['multiple' => true, 'class' => 'form-control selectpicker']) !!}
            </div>
            <div class="border rounded p-3 bg-light form-group">
                ...
                @foreach($orderDetailStates as $id => $orderDetailStateName)
                    <div class="form-group">
                        ...
                        {!! Form::select("new_order_detail_states[{$id}]", $orderDetailStates, $routeListState->newOrderDetailStateId($id), ['class' => 'form-control selectpicker']) !!}
                    </div>
                @endforeach
            </div>
        </div>
        <div class="col-xs-12 col-sm-6">
            <div class="form-group">
                ...
                {!! Form::select('need_order_state_id[]', $orderStates, $routeListState->needOrderStates->pluck('id'), ['multiple' => true, 'class' => 'form-control selectpicker']) !!}
            </div>
            <div class="border rounded p-3 bg-light form-group">
                ...
                @foreach($orderStates as $id => $orderStateName)
                    <div class="form-group">
                        ...
                        {!! Form::select("new_order_states[{$id}]", $orderStates, $routeListState->newOrderStateId($id), ['class' => 'form-control selectpicker']) !!}
                    </div>
                @endforeach
            </div>
            <div class="form-group">
                {!! Form::label('need_product_return_state_id[]', __('Need Product Return states:')) !!}
                {!! Form::select('need_product_return_state_id[]', $productReturnStates, $routeListState->needProductReturnStates->pluck('id'), ['multiple' => true, 'class' => 'form-control selectpicker']) !!}
            </div>
            <div class="border rounded p-3 bg-light form-group">
                ...
                @foreach($productReturnStates as $id => $productReturnStateName)
                    <div class="form-group">
                        ...
                        {!! Form::select("new_product_return_states[{$id}]", $productReturnStates, $routeListState->newProductReturnStateId($id), ['class' => 'form-control selectpicker']) !!}
                    </div>
                @endforeach
            </div>
            <div class="form-group">
                {!! Form::label('need_product_exchange_state_id[]', __('Need Product Exchange states:')) !!}
                {!! Form::select('need_product_exchange_state_id[]', $productExchangeStates, $routeListState->needProductExchangeStates->pluck('id'), ['multiple' => true, 'class' => 'form-control selectpicker']) !!}
            </div>
            <div class="border rounded p-3 bg-light form-group">
                ...
                @foreach($productExchangeStates as $id => $productExchangeStateName)
                    <div class="form-group">
                        ...
                        {!! Form::select("new_product_exchange_states[{$id}]", $productExchangeStates, $routeListState->newProductExchangeStateId($id), ['class' => 'form-control selectpicker']) !!}
                    </div>
                @endforeach
            </div>
        </div>
