...
@section('content')
    {!! Form::open(['route' => 'product-exchanges.store','method'=>'POST']) !!}
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h4 class="float-left mr-3">{{ __('Create New Exchange to Order') }} {{ $order->getDisplayNumber() }}</h4>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6">
            <div class="form-group row">
                <div class="col-xs-12 col-sm-4" id="deliveryDate">
                    ...
                    {!! Form::text('delivery_estimated_date', null, ['class' => 'form-control date', 'autocomplete' => 'off']) !!}
                </div>
        ...
        <table class="table table-bordered table-sm m-3">
            ...
            <tbody>
            @foreach($availableOrderDetails as $orderDetail)
                <tr data-item="order-detail" data-status="not-init" class="order-detail">
                    ...
                    <td>
                        ...
                        $orderDetail->price,
                        ['class' => 'form-control', 'size'=> 6, 'disabled' => true]
                        ) !!}
                    </td>
                    ...
                    <td class="product-store">
                        ...
                        ['class' => 'selectpicker-order-detail-simple',  'data-item' => 'order-detail-store']
                        ) !!}
                    </td>
        ...
        <table class="table table-bordered table-sm m-3 not-hidden">
            ...
            <tr data-item="order-detail" data-status="not-init" class="order-detail">
                <td class="product-name" style="width: 50%;">
                    {!! Form::select(
                    'order_detail_add[0][product_id]',
                    $products,
                    [],
                    ['class' => 'selectpicker-order-detail', 'data-item' => 'order-detail-product']
                    ) !!}
                </td>
                <td class="certificate-number">{!! Form::text('order_detail_add[0][certificate_number]', $orderDetail->product->category->is_certificate ? $orderDetail->certificate->number : 0, ['class' => 'form-control']) !!}</td>
                <td class="empty-td"></td>
                <td data-item="order-detail-reference"></td>
                <td>
                    {!! Form::text(
                    'order_detail_add[0][price]',
                    null,
                ...
                <td class="product-currency">
                    {!! Form::select(
                    'order_detail_add[0][currency_id]',
                    $currencies,
                    [],
                    ['class' => 'selectpicker-order-detail-simple']
                    )!!}
                ...
                <td class="product-state">
                    {!! Form::select(
                    'order_detail_add[0][order_detail_state_id]',
