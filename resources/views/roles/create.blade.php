    ...
    <div class="row">
        <div class="col-lg-12 margin-tb">
            ...
            <div class="pull-right">
                <a class="btn btn-sm btn-primary" href="{{ route('roles.index') }}"> {{ __('Back') }}</a>
            </div>
        </div>
    </div>
                ...
                @foreach($permission->groupBy(function ($item) {return explode('-', $item->name)[0];})->sortBy(function ($item, $key) {return $key;}) as $permissionGroup)
                    <div class="row box-placeholder">
                        @foreach($permissionGroup as $value)
                            <label class="col-xs-12 col-sm-3">{{ Form::checkbox('permission[]', $value->id, false, array('class' => 'name')) }}
                                {{ $value->name }}</label>
                        @endforeach
                    </div>
                @endforeach
